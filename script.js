// script.js — Tam dosya
// 25 soru, 30s sayaç, 2 pas, +5 / 0 / -2 puan, yanlışta doğruyu 2s kırmızı göster, ambient müzik, pas sesi, branş görseli
// Ek: emoji öne ekleme, 'evet'/'hayir' esnek kabul, cevap kutusu ortada, branch görsel küçültüldü
...
function skipQuestion(){
  ...
  passCount++;
  el('passesLeft').textContent = passesLeft;
  showFeedback(false, 'Pas verildi. 0 puan.');
  setTimeout(()=>{
    current++;
    loadQuestion();
  },700);
...
function endGame(){
  ...
  saveScore({name:playerName, cls:playerClass, score:score, correct:correctCount, pass:passCount, wrong:wrongCount, date: new Date().toLocaleString('tr-TR')});
  renderRecentScores();
...
/* Skor kaydet ve göster */
function saveScore(obj){
  const key = 'ykssim_scores_v6';
...
function renderRecentScores(){
  ...
  const recentEl = el('recentScores');
  const listEl = el('scoreList');

  const html = recent.length ? '<ul>' + recent.map(s=>`<li>${s.date} — ${s.name} (${s.cls}) → ${s.score} puan (D:${s.correct} P:${s.pass} Y:${s.wrong})</li>`).join('') + '</ul>' : '<p class="small">Henüz skor yok.</p>';
